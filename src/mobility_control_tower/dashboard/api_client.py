...
ENDPOINTS = {
    ...
    "incidents": "/incidents",
    "incident_evaluations": "/v1/incidents/evaluations",
...
def get_json(
    ...
    try:
        if headers:
            response = requests.get(_url(api_url, endpoint), params=params, headers=headers, timeout=timeout)
        else:
...
def post_json(api_url: str, endpoint: str, token: str, params: dict[str, Any] | None = None, timeout: int = 5) -> dict[str, Any]:
    headers = {"Authorization": f"Bearer {token}"} if token else {}
    try:
        response = requests.post(_url(api_url, endpoint), json=params, headers=headers, timeout=timeout)
    except requests.RequestException as exc:
        return {"ok": False, "error": f"API is not reachable at {api_url}. Details: {exc}", "data": [], "count": 0}
    if response.status_code >= 400:
        try:
...
def fetch_dashboard_data(api_url: str) -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        "sources": get_json(api_url, ENDPOINTS["sources"]),
        "network_status": get_json(api_url, ENDPOINTS["network_status"], {"limit": 20}),
        "network_reliability": get_json(api_url, ENDPOINTS["network_reliability"], {"limit": 20}),
        "routes_api": get_json(api_url, ENDPOINTS["routes_api"], {"limit": 100}),
        "vehicles": get_json(api_url, ENDPOINTS["vehicles"], {"limit": 100}),
        "alerts_active": get_json(api_url, ENDPOINTS["alerts_active"], {"limit": 100}),
        "incidents": get_json(api_url, ENDPOINTS["incidents"], {"limit": 100}),
        "incident_evaluations": get_json(api_url, ENDPOINTS["incident_evaluations"], {"limit": 10}),
        "data_quality_status": get_json(api_url, ENDPOINTS["data_quality_status"]),
        "lineage_status": get_json(api_url, ENDPOINTS["lineage_status"]),
    }
